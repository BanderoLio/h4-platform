...
ReAct-оркестратора, где модель сама выбирала, кого вызвать.
    ...
    START → intake → (clarify?) → recon ─┬─→ specialist_injection ─┐
                              ...
                              consolidate ←──────────────────────┘
                                   │
                              validate → gate → report → patches → END
"""
...
from .patching import generate_fix_patches
...
def _patches(state: AnalysisState) -> dict:
    """Генерирует candidate-патчи по подтверждённым находкам."""
    if not CONFIG.generate_fix_patches:
        return {"fix_patches": []}
    findings = state.get("validated_findings", []) or []
    if not findings:
        return {"fix_patches": []}
    ...
    except Exception as err:  # noqa: BLE001 — деградация шага не валит весь граф
        _log(f"[patches] ошибка генерации: {err}")
        return {
            "fix_patches": [],
            "errors": [f"patch-generator: {str(err)[:200]}"],
        }
    with_diff = sum(1 for p in patches if p.get("unified_diff"))
    _log(f"[patches] сгенерировано {len(patches)} патчей ({with_diff} с diff)")
    return {"fix_patches": patches}
...
def build_graph(checkpointer=None):
    ...
    graph.add_node("patches", _patches)
    ...
    graph.add_edge("report", "patches")
    graph.add_edge("patches", END)
