...
class Config:
    ...
    # Прогонять semgrep по файлам-кандидатам в фазе index — источник
    # точных кандидатов триажа. Кэшируется по git-commit.
    index_scanners: bool = True
    # Лимит времени на прогон сканера в фазе index (сек).
    scanner_timeout_sec: int = 240
