...
- **patches** — candidate-патчи (unified diff) для подтверждённых находок.
...
Для патч-этапа доступны `--no-patches` (полностью отключить генерацию) и
`--patch-max N` (ограничить число находок, для которых строятся candidate-patch).
...
машиночитаемый JSON. Если по находкам сгенерированы candidate-патчи, рядом
пишутся `*.patches.md` (человеко-читаемое объяснение) и `*.patches.diff`
(единый diff-бандл). Когда ветка B добавит `Finding`, функция уже принимает
...
agentsec/
  ...
  patching.py                генерация candidate-patch для подтверждённых finding
